...
def callbak_query(call):
    ...
    if call.data == 'ar':
        bot.answer_callback_query(call.id, "اللغه الان هي العربي ✅ .")
        ...
        bot.answer_callback_query(call.id, " Язык теперь русский ✅.")
        outfile = open( 'user_data/'+ path_us , 'w')
        outfile.write( 'ru' )
        bot.send_message(call.from_user.id, text=ru_help , reply_markup= markup_inline_help())
